...
class TestPPU(unittest.TestCase):
    ...
    def test_ppu_tick(self):
        ...
        assert len(pt_data) == 960
        assert len(at_data) == 64
        nt = bytearray(pt_data) + bytearray(at_data)
        assert len(nt) == 1024
        # TODO: (1) build test nametable (2) copy into vram using basic mirroring at $2000 - $2FFF
        # Vertical mirroring: $2000 equals $2800 and $2400 equals $2C00
        # vram[0x2000] = 0x0
        # TODO: copy test palettes into vram $3F00-$3F1F
        ...
        ppu_tick, ppu_read_reg, ppu_write_reg, ppu_write_oam, ppu_pals, ppu_connect, ppu_inspect_regs = \
            self._build_ppu_funcs(b'', chr_rom=chr_rom, t=30000)
        # Cycle 0
    ...
    def test_render_nametable_with_pygame(self):
        ...
        pygame.display.init()
        test_pals = [
            [((i*(255//3)), (i*(255//3)), (i*(255//3))) for i in range(4)],
            [((0), (i*(255//3)), (i*(255//3))) for i in range(4)],
            [((i*(255//3)), (0), (i*(255//3))) for i in range(4)],
            [((i*(255//3)), (i*(255//3)), (0)) for i in range(4)],
        ]
        table_idx = 1
            ...
            with pygame.PixelArray(screen) as pixels:
                for row in range(nt_rows):
                    for col in range(nt_cols):
                        ...
                        pt_value = nt[pt_idx]
                        at_quad = nt[960+at_idx] # attr_quad = fetch(0x23C0|(ppu_addr&0x0C00)|(((ppu_addr&0x0380)>>4)|((ppu_addr&0x001C)>>2)))
                        ...
                        for fine_y in range(0, 8):
                            for fine_x in range(0, 8):
                                ...
                                pixels[x+fine_x,y+fine_y] = test_pals[pal_idx][col_idx]
